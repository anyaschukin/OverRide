...
Dump of assembler code for function test:
   ...
   0x08048759 <+18>:	mov    %eax,-0xc(%ebp)                             ; key = 322424845 - input
   0x0804875c <+21>:	cmpl   $0x15,-0xc(%ebp)                            ; if key > 21
   ...
   0x0804877b <+52>:	call   0x8048660 <decrypt>                         ; decrypt(key)
...
Dump of assembler code for function decrypt:
   ...
   0x0804869c <+60>:	lea    -0x1d(%ebp),%eax                            ; load crypt into eax
   ...
   0x080486b0 <+80>:	mov    %edx,%edi                                   ; load crypt into edi
   0x080486b2 <+82>:	repnz scas %es:(%edi),%al                          ; strlen(crypt)
   0x080486b4 <+84>:	mov    %ecx,%eax
   0x080486b6 <+86>:	not    %eax
   0x080486b8 <+88>:	sub    $0x1,%eax
   0x080486bb <+91>:	mov    %eax,-0x24(%ebp)                            ; size_t len = strlen(crypt)
   0x080486be <+94>:	movl   $0x0,-0x28(%ebp)                            ; int i = 0
   0x080486c5 <+101>:	jmp    0x80486e5 <decrypt+133>
   
   0x080486c7 <+103>:	lea    -0x1d(%ebp),%eax                         ; crypt
   0x080486ca <+106>:	add    -0x28(%ebp),%eax                         ; 32 bytes? index?
   0x080486cd <+109>:	movzbl (%eax),%eax                              ; return crypt[i]
